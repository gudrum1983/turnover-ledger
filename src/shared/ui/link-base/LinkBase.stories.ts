...
const meta = {
  ...
  parameters: {
    docs: {
      description: {
        component:
          'Базовая внутренняя ссылка на `RouterLink`. Компонент принимает маршрут через `to`, поддерживает размеры текста и наследует состояние hover из дизайн-токенов. Используется только для навигации внутри приложения; для внешних URL и `mailto:` предназначены `LinkExternal` и `LinkEmail`.',
      },
    },
  },
  component: LinkBase,
...
export const Playground: Story = {
  ...
  parameters: {
    docs: {
      description: {
        story: 'Интерактивная история для проверки размера и текста внутренней ссылки внутри приложения.',
      },
    },
  },
}
